...
def to_log (logfile, messages):
    #print('thread %s >>> (%s)' % (threading.current_thread().name, 'ok'))
    with open(logfile, "a") as f:
        print(messages, file=f)
        
def long_time_task(ftime, file, step, dblink):
    #print('Run task %s (%s)...' % (ftime, threading.active_count()))
    print('thread %s of %s>>> %s' % (threading.current_thread().name, threading.active_count(), ftime))
    pool.submit(to_log, "output.log", 'thread %s of %s>>> %s' % (threading.current_thread().name, threading.active_count(), ftime))
    #pool.submit(to_log, "output.log", 'thread %s >>> %s' % (threading.current_thread().name, ftime))
    pool.submit(to_log, "output.log", "sqlplus " + dblink + " @" + file + " " + ftime +" " + step+ " > nul")
    start = time.time()
    ...
    pool.submit(to_log, "output.log", 'Task %s runs %0.2f seconds.' % (ftime, (end - start)))
    #return res
...
def spooldb(file, dblink, starttime, endtime, step, maxworks):
    ...
    pool.submit(to_log, "output.log", 'All subprocesses done.')
    pool.submit(to_log, "output.log", 'Task all runs %0.2f seconds.' % ((end - start)))  
...
if __name__ == '__main__':
    try:
        ...
        print(e)
        pool.submit(to_log, "output.log", e)
```
